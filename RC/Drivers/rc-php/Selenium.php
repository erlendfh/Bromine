...
class Testing_Selenium
{
    ...
    */
    ...
    public function __construct($s_id, $browser, $browserUrl, $testName, $host, $verification = "verifyNoneFailed", $port = 4444, $timeout = 60000)
    {
        $this->s_id = $s_id;
        ...
        $this->test = new Test($s_id,$testName, $verification, $u_id);     
        $this->t_id = $this->test->createTest();   
    }
    ...
    public function verifyText($locator, $assertion){
      $getText = $this->getText($locator);
      $arr = array($locator, $assertion);
      
      if ($getText != $assertion){
        $c = new Command($this->t_id,"verifyText", $arr, 'OK', 'failed');
        $this->captureScreenshot();
      }
      else{
        $c = new Command($this->t_id,"verifyText", $arr, 'OK', 'passed');
      }
      $this->test->addResult($c);
    }

    
    public function verifyValue($locator, $assertion){
      $getText = $this->getValue($locator);
      $arr = array($locator, $assertion);
      //echo "GetText:".$getText."<br />";
      //echo "Assert:".$assertion."<br />";
      
      if ($getText != $assertion){
        $c = new Command($this->t_id,"verifyValue", $arr, 'OK', 'failed');

      }
      else{
        $c = new Command($this->t_id,"verifyValue", $arr, 'OK', 'passed');
      }
      $this->test->addResult($c);
    }
    ...
    {
        //$absolutePath = "../ss/".date('Y-m-d-H-i-s').".png";
        
        $ss=str_replace('/','\\',$_GET['ss']);
        $absolutePath = $ss."\\ss\\".date('Y-m-d-H-i-s').".png";
        
        //echo $_GET['ss']."<br />";
        //echo $absolutePath."<br />";
        $this->doCommand("captureScreenshot", array($absolutePath));
    }
  
    public function deencodeURL($str){
      $str = str_replace("%E6", "æ", $str);
      $str = str_replace("%C6", "Æ", $str);
      $str = str_replace("%F8", "ø", $str);
      $str = str_replace("%D8", "Ø", $str);
      $str = str_replace("%E5", "å", $str);
      $str = str_replace("%C5", "Å", $str);
      //echo $str."<br />";
      return $str;
    }
    ...
    protected function doCommand($verb, $args = array(), $forceStatus=false, $forceScreenshot=null){
        ...
        for ($i = 0; $i < count($args); $i++) {
            $argNum = strval($i + 1);
            $url .= sprintf('&%s=%s', $argNum, $this->deencodeURL(urlencode(trim($args[$i]))));
        }
        ...
        if (!$handle = fopen($url, 'r')) {
            throw new Exception('Cannot connect to Selenium RC Server');
        }
    ...
    public function customCommand($command, $var1, $var2, $status){
      $la = array($var1, $var2);
      $c = new Command($this->t_id,$command, $la, "OK", $status);
      $this->test->addResult($c);
      if($status=='failed'){
        $this->captureScreenshot();
      }
