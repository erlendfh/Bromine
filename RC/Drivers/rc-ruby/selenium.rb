...
require 'uri'
require 'cgi'
require '../../../Drivers/RC-RUBY/command.rb'
require '../../../Drivers/RC-RUBY/test.rb'
...
module Selenium
    ...
    class SeleniumDriver
        ...
        def initialize(server_host, browserStartCommand, browserURL, timeout, testName, logFile, verification = "verifyNoneFailed", server_port = 4444)
            @server_host = server_host
            @server_port = server_port
            @browserStartCommand = browserStartCommand
            @browserURL = browserURL
            @timeout = timeout
            @test = Test.new(testName, verification, logFile)
        ...
        def verifyText(locator, assertion)
            getText = getText(locator)
            arr = [locator, assertion]
            if(getText != assertion)
              c = Command.new("verifyText", arr, 'OK', 'failed')
            else
              c = Command.new("verifyText", arr, 'OK', "passed")
            end
            @test.addResult(c)
        end
        ...
        def do_command(verb, args)
		   ...
		   if(args.kind_of? Array)
			if(args.size() == 1)	
			    args2 = Array.new()
			    args2 = args
			    args2[1] = response.body
			    c = Command.new(verb, args2, response.body, "failed")
			    @test.addResult(c)
			    raise SeleniumCommandError, response.body
			else

			    args[1] += response.body
			    c = Command.new(verb, args, response.body, "failed")
		            @test.addResult(c)
		            raise SeleniumCommandError, response.body
			end
		    end
