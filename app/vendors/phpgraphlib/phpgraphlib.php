...
class PHPGraphLib
{
	...
	function createGraph() //MAIN CLASS METHOD - CALLED LAST
	{
		...
		$this->displayErrors();
		//OUTPUT TO BROWSER
		//Ralle modification: output to file, retrn path. Enables in-viewer usage.
		$time = time();
		//Clean up temp dir
		$files = scandir('img'.DS.'temp'.DS);
		foreach($files as $file){ 
            if(is_file("img".DS."temp".DS.$file) && current(explode('.',$file))<($time-60)){ //Delete all pics more than 1 minute old 
                unlink("img".DS."temp".DS.$file);
            }
        }
